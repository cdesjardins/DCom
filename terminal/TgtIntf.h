...
protected:
    enum eTgtSshAuthState
    {
        TGT_SSH_AUTH_STATE_USER_PROMPT,
        TGT_SSH_AUTH_STATE_USER_ACCEPT,
        TGT_SSH_AUTH_STATE_PASS_PROMPT,
        TGT_SSH_AUTH_STATE_PASS_ACCEPT,
        TGT_SSH_AUTH_STATE_PASS_AUTHENTICATE,
        TGT_SSH_AUTH_STATE_DONE,
        TGT_SSH_AUTH_STATE_FAILED,
    } ;

    virtual char * TgtMakeConnection();
    int TgtAuthenticatePrompt(char *szReadData, int nMaxBytes);
    int TgtAuthenticateAccept(char *szWriteData, int nBytes);
    int TgtAuthenticate(char *szReadData, int nMaxBytes);
    int TgtAuthenticationFeedback(char *szReadData, char *szInput, bool bPassword);
    int TgtAuthenticateAcceptInput(char *szWriteData, int nBytes, char *szFinalInput, eTgtSshAuthState eNextState);
    void ResetAuthData(bool bClearUserPass, eTgtSshAuthState eAuthState);

    ssh_session m_sshSession;
    ssh_channel m_sshChannel;
    TgtConnection m_sTgtConnection;
    eTgtSshAuthState m_eAuthenticated;
    int m_nAuthShown;
    int m_nAuthLen;
    char m_szAuthInput[256];
    char m_szUsername[256];
    char m_szPassword[256];
    CScrollBufL1 m_cbUserPass;
};
